...
export default function TimeOffPage() {
  ...
  const [handoffItems, setHandoffItems] = useState<string[]>(["Share active project status and deadlines", "Transfer urgent approvals and owner context"]);
  ...
  const duration = useMemo(() => countBusinessDays(form.startDate, form.endDate, effectivePortion), [effectivePortion, form.endDate, form.startDate]);
  ...
  const impactLevel = duration >= 10 ? "High impact" : duration >= 5 ? "Moderate impact" : duration > 0 ? "Low impact" : "Pending";
  ...
  const readinessChecks = useMemo(
    ...
    [balanceOk, dateError, duration, form.acknowledged, form.delegate, form.endDate, form.reason, form.startDate, handoffItems],
  );
  ...
  const warnings = useMemo(() => {
    ...
    if (duration > 0 && !balanceOk) nextWarnings.push("This request exceeds the available balance for the selected leave type.");
  ...
  return (
    <div className="text-slate-900">
      ...
      {submitState === "success" && (
        <div aria-live="polite" className="mb-6 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm text-emerald-800">
      ...
      <div className="gap-6 ">
        <form onSubmit={handleSubmit} className="space-y-6">
          <SectionCard title="Request details" subtitle="Choose the leave type, define the schedule, and provide context for the approver.">
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl border p-4 text-left transition ${
                      selected ? "border-cyan-300 bg-cyan-50 shadow-sm" : "border-slate-200 bg-white hover:border-slate-300 hover:bg-slate-50"
                    }`}>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              <div>
                ...
                <input type="date" value={form.startDate} onChange={(event) => updateField("startDate", event.target.value)} className={inputClassName} />
              </div>

              <div>
                <label className="text-sm font-medium text-slate-700">End date</label>
                <input type="date" value={form.endDate} onChange={(event) => updateField("endDate", event.target.value)} className={inputClassName} />
              </div>
              ...
              <div>
                ...
                <p className="mt-2 text-xs text-slate-500">Half-day requests are available for single-day requests only.</p>
              </div>

              <div>
                <label className="text-sm font-medium text-slate-700">Approving manager</label>
                <select value={form.manager} onChange={(event) => updateField("manager", event.target.value)} className={inputClassName}>
            ...
            <div className="mt-4">
              ...
              <div className="mt-2 flex items-center justify-between gap-3 text-xs">
                ...
                <span className={form.reason.trim().length >= 12 ? "text-emerald-600" : "text-slate-400"}>{form.reason.trim().length} characters</span>
              </div>
            ...
            <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">Duration is calculated in business days and excludes weekends.</p>
                </div>
                <div className="text-sm text-slate-700">
                  {form.startDate && form.endDate ? `${formatDisplayDate(form.startDate)} → ${formatDisplayDate(form.endDate)}` : "Select a date range"}
                </div>
              </div>

              <div className="mt-3 flex flex-wrap items-center gap-3 text-xs">
                {dateError ? <span className="rounded-full border border-rose-200 bg-rose-50 px-3 py-1 text-rose-700">{dateError}</span> : null}
                {hasWeekendSelection ? (
          ...
          <SectionCard title="Coverage and handoff" subtitle="Document who will support urgent work and what needs to be transferred before time off.">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="text-sm font-medium text-slate-700">Primary delegate</label>
                <select value={form.delegate} onChange={(event) => updateField("delegate", event.target.value)} className={inputClassName}>
          ...
          <SectionCard title="Additional notes" subtitle="Attach supporting details, workflow links, or extra information for the approval chain.">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
            ...
            <label className="mt-4 flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              />
              <div>
                <p className="text-sm font-medium text-slate-900">I reviewed balance, coverage, and approval requirements.</p>
                <p className="mt-1 text-xs text-slate-500">Some leave types may require HR review or additional notice before approval.</p>
              </div>
            </label>

            {attemptedSubmit && !form.acknowledged && <p className="mt-3 text-sm text-rose-600">Acknowledge the policy requirements before submitting.</p>}

            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="text-xs text-slate-500">
                Submission status:{" "}
                <span className="font-medium text-slate-700">{submitState === "idle" ? "Draft" : submitState === "submitting" ? "Submitting" : "Ready"}</span>
              </div>
        ...
        <aside className="space-y-6">
          <SectionCard title="Live summary" subtitle="A preview of the request that the reviewer will receive.">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-medium text-slate-900">{leaveTypes.find((item) => item.value === form.type)?.label}</p>
                  <p className="mt-1 text-xs text-slate-500">
                    {form.startDate && form.endDate ? `${formatDisplayDate(form.startDate)} → ${formatDisplayDate(form.endDate)}` : "Date range pending"}
                  </p>
                </div>
                <span
              ...
              <div className="mt-4 space-y-3 text-sm">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <span className="font-medium text-slate-900">{duration > 0 ? `${formatDays(duration)} day${duration === 1 ? "" : "s"}` : "—"}</span>
                </div>
                <div className="flex items-center justify-between gap-4">
            ...
            <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-700">{form.reason.trim() || "Add request context to preview the approver note."}</p>
            </div>
          ...
          <SectionCard title="Readiness score" subtitle="A quick signal showing whether the request is complete enough to send.">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="flex items-end justify-between gap-4">
                ...
                <p className="max-w-[11rem] text-right text-xs leading-5 text-slate-500">Complete the remaining checks to improve approval readiness.</p>
              </div>
              <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-200">
                <div className={`h-full rounded-full transition-all ${readinessBarClass}`} style={{ width: `${readiness}%` }} />
              </div>
                  ...
                  <div key={item.label} className="flex items-center justify-between gap-3 text-sm">
                    ...
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-medium ${
                        item.done ? "border border-emerald-200 bg-emerald-50 text-emerald-700" : "border border-slate-200 bg-white text-slate-500"
                      }`}>
          ...
          <SectionCard title="Balance and policy" subtitle="Current entitlement, post-request balance, and guidance for the selected leave type.">
            <div className="space-y-3">
              {leaveTypes.map((type) => {
                ...
                return (
                  <div key={type.value} className={`rounded-2xl border p-4 ${selected ? "border-cyan-200 bg-cyan-50" : "border-slate-200 bg-slate-50"}`}>
                    <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center justify-between gap-4">
                ...
                <span
                  className={`text-sm font-medium ${
                    remainingBalance === null ? "text-slate-700" : remainingBalance >= 0 ? "text-emerald-700" : "text-rose-700"
                  }`}>

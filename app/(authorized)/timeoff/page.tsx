...
const policyNotes: Record<TimeOffType, string> = {
  vacation:
    "Vacation requests are typically approved based on team capacity and notice period.",
  ...
  parental:
    "Parental leave often requires an HR workflow and extended handoff planning.",
...
export default function TimeOffPage() {
  ...
  const returnDate = useMemo(
    () => getNextBusinessDay(form.endDate),
    [form.endDate],
  );
  ...
  const remainingBalance =
    form.type === "unpaid"
      ? null
      : Math.round((balances[form.type] - duration) * 10) / 10;
    ...
    () => [
      {
        label: "Dates selected",
          ...
          form.startDate && form.endDate && !dateError && duration > 0,
        ),
      },
      {
  ...
  const warnings = useMemo(() => {
    ...
    if (!form.startDate || !form.endDate)
      nextWarnings.push("Select a valid time window.");
    if (dateError) nextWarnings.push(dateError);
    if (duration > 0 && !balanceOk)
      nextWarnings.push(
        "This request exceeds the available balance for the selected leave type.",
      );
    if (form.reason.trim().length < 12)
      nextWarnings.push("Add more context to the request reason.");
    if (!handoffItems.some((item) => item.trim().length > 0))
      nextWarnings.push("Add at least one handoff item.");
    if (!form.acknowledged)
      nextWarnings.push(
        "Confirm the policy acknowledgement before submitting.",
      );
    ...
    <div className="min-h-screen text-slate-900">
      <div className="mx-auto max-w-7xl px-6 py-10">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
                Plan leave, coordinate coverage, and preview approval details
                before submitting.
              </p>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm shadow-slate-200/40">
                ...
                <p className="mt-2 text-2xl font-semibold text-slate-900">
                  {formatDays(balances[form.type])} days
                </p>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm shadow-slate-200/40">
                ...
                <p className="mt-2 text-2xl font-semibold text-slate-900">
                  {duration > 0 ? `${formatDays(duration)}d` : "—"}
                </p>
                <p className="mt-1 text-xs text-slate-500">
                  {isSingleDay && effectivePortion !== "full"
                    ? "Half-day request"
                    : "Business days only"}
                </p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm shadow-slate-200/40">
                ...
                <p className="mt-2 text-2xl font-semibold text-slate-900">
                  {form.endDate
                    ? formatDisplayDateFromDate(returnDate).split(",")[0]
                    : "—"}
                </p>
                <p className="mt-1 text-xs text-slate-500">
                  {form.endDate
                    ? formatDisplayDateFromDate(returnDate)
                    : "Select an end date"}
                </p>
              </div>
            </div>
        ...
        {submitState === "success" && (
          ...
          >
            Request prepared successfully. Connect the submit handler to the API
            or workflow when ready.
          </div>
        )}
        ...
        <div className="grid gap-6 lg:grid-cols-[1.6fr,0.9fr]">
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateField("portion", event.target.value as Portion)
                    }
                  ...
                  <p className="mt-2 text-xs text-slate-500">
                    Half-day requests are available for single-day requests
                    only.
                  </p>
                </div>
              ...
              <div className="mt-4">
                ...
                <div className="mt-2 flex items-center justify-between gap-3 text-xs">
                  <span className="text-slate-500">
                    A short explanation helps reviewers approve faster.
                  </span>
                  <span
                    className={
                      form.reason.trim().length >= 12
                ...
                {attemptedSubmit && form.reason.trim().length < 12 && (
                  <p className="mt-2 text-sm text-rose-600">
                    Add at least 12 characters to describe the request.
                  </p>
                )}
              </div>
              ...
              <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      Duration is calculated in business days and excludes
                      weekends.
                    </p>
                  </div>
            ...
            >
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <select
                    value={form.delegate}
                    onChange={(event) =>
                      updateField("delegate", event.target.value)
                    }
                    className={inputClassName}
                  >
                  ...
                  {handoffItems.map((item, index) => (
                    <div key={`${index}-${item}`} className="flex gap-3">
                      <input
                        ...
                        value={item}
                        onChange={(event) =>
                          updateHandoffItem(index, event.target.value)
                        }
                ...
                {attemptedSubmit &&
                  !handoffItems.some((item) => item.trim().length > 0) && (
                    <p className="mt-3 text-sm text-rose-600">
                      Add at least one handoff item before submitting.
                    </p>
                  )}
              </div>
              ...
              <div className="mt-4">
                <label className="text-sm font-medium text-slate-700">
                  Critical blockers or deadlines
                </label>
                <textarea
                  value={form.blockers}
                  onChange={(event) =>
                ...
                <label className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-start gap-3">
                    <input
                      ...
                      onChange={(event) =>
                        updateField("reachable", event.target.checked)
                      }
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Allow urgent contact while away if required.
                      </p>
                    </div>
                  </div>
                </label>
                ...
                <label className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-start gap-3">
                    <input
                      ...
                      onChange={(event) =>
                        updateField("notifyTeam", event.target.checked)
                      }
                      className="mt-1 h-4 w-4 rounded border-slate-300 bg-white text-cyan-600 focus:ring-cyan-500"
                    />
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Share approved dates with affected teammates.
                      </p>
                    </div>
                  </div>
                </label>
                ...
                <label className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-start gap-3">
                    <input
                      ...
                      onChange={(event) =>
                        updateField("createCalendarHold", event.target.checked)
                      }
                      className="mt-1 h-4 w-4 rounded border-slate-300 bg-white text-cyan-600 focus:ring-cyan-500"
                    />
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Block focus calendar time for the full request.
                      </p>
                    </div>
                  </div>
                </label>
            ...
            >
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateField("documentLink", event.target.value)
                    }
                    placeholder="https://"
                    className={inputClassName}
                  />
                  <p className="mt-2 text-xs text-slate-500">
                    Optional: shared doc, leave plan, or HR case reference.
                  </p>
                </div>
              ...
              <label className="mt-4 flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Some leave types may require HR review or additional notice
                    before approval.
                  </p>
                </div>
              </label>
              ...
              {attemptedSubmit && !form.acknowledged && (
                <p className="mt-3 text-sm text-rose-600">
                  Acknowledge the policy requirements before submitting.
                </p>
              )}
          ...
          <aside className="space-y-6">
            <SectionCard
              title="Live summary"
              subtitle="A preview of the request that the reviewer will receive."
            >
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-sm font-medium text-slate-900">
                      {
                        leaveTypes.find((item) => item.value === form.type)
                          ?.label
                      }
                    </p>
                    <p className="mt-1 text-xs text-slate-500">
                ...
                <div className="mt-4 space-y-3 text-sm">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <span className="font-medium text-slate-900">
                      {duration > 0
                        ? `${formatDays(duration)} day${duration === 1 ? "" : "s"}`
                        : "—"}
                    </span>
                  </div>
                  <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <span className="font-medium text-slate-900">
                      {form.delegate || "Not assigned"}
                    </span>
                  </div>
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-slate-500">Urgent availability</span>
                    <span className="font-medium text-slate-900">
                      {form.reachable ? "Available" : "Offline"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-700">
                  {form.reason.trim() ||
                    "Add request context to preview the approver note."}
                </p>
              </div>
              ...
              <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex items-center justify-between gap-4">
                  ...
                  >
                    {remainingBalance === null
                      ? "Policy based"
                      : `${formatDays(remainingBalance)} days`}
                  </span>
                </div>
                <p className="mt-3 text-sm leading-6 text-slate-700">
                  {policyNotes[form.type]}
                </p>
              </div>
            </SectionCard>
            ...
            <SectionCard
              title="Alerts"
              subtitle="Items that may block approval or require follow-up."
            >
              {warnings.length > 0 ? (
                <div className="space-y-3">
                ...
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
                  No blocking issues detected. The request looks ready to
                  submit.
                </div>
              )}
            </SectionCard>

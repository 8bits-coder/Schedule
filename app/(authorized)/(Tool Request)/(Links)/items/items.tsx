...
const ShowAllItems = async () => {
  const { data, serverError } = await FetchAllItems();

  if (serverError) {
    return <div className="text-sm text-destructive">Error: {serverError}</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <p>
        Total: <span className="text-red-600">{data.length}</span> {data.length === 1 ? "item" : "items"}
      </p>
      <div className="grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-4">
        {data.map((item) => (
          <div
            ...
            <Link href={Links.Items + `/edit/${item.id}`}>

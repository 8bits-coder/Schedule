...
export default function DeliveryPage() {
  ...
  const [receivedPerson, setReceivedPerson] = useState<{ id: string; name: string }[]>([]);
  const [formData, setFormData] = useState<Omit<DeliveryReceipt, "id" | "createdAt" | "updatedAt"> & { [key: string]: any }>({
    itemId: "",
    itemName: "",
    itemSerialNumber: "",
    deliveryPersonId: "",
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <Select
          name="itemId"
          onValueChange={(value) => setFormData((prev) => ({ ...prev, itemId: value || "", itemName: items.find((item) => item.id === value)?.name || "" }))}
        ...
        <Input type="text" placeholder="Serial Number" name="itemSerialNumber" value={formData.itemSerialNumber} onChange={handleChange} required />

        <Select
          name="workLocationId"
          onValueChange={(value) => {
            setFormData((prev) => ({ ...prev, workLocationId: value || "", workLocationName: workLocations.find((loc) => loc.id === value)?.name || "" }));
          }}
